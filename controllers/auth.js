    ...
    jwt.verify(token, process.env.JWT_SECRET, function (err, decoded) {
      if (err) {
        ...
        return res.status(401).json({
          error: "Link kadaluwarsa. Silahkan coba lagi!",
        });
      }
      ...
      user.save((err, user) => {
        if (err) {
          ...
          return res.status(401).json({
            error: "Error saat menyimpan data. Coba lagi!",
          });
        }
        console.log(user);
        return res.json({
          message: "Daftar berhasil. Silahkan masuk.",
        });
      });
    });
  } else {
...
exports.signin = (req, res) => {
  ...
  User.findOne({ email }).exec((err, user) => {
    ...
    if (err || !user) {
      return res.status(401).json({
        error: "Email tidak terdaftar. Silahkan daftar dahulu.",
      });
    }
...
exports.isAdmin = (req, res, next) => {
  if (req.profile.role === 0) {
    return res.status(403).json({
      error: "Untuk admin! Akses dilarang",
    });
  }
  next();
};
    ...
    return user.updateOne({ resetPasswordLink: token }, (err, success) => {
      if (err) {
        ...
        return res.status(200).json({
          message: `Email telah dikirim ke ${email}. Ikuti langkah selanjutnya.`,
        });
      }
    });
  });
...
exports.resetPassword = (req, res) => {
  ...
  if (resetPasswordLink) {
    jwt.verify(resetPasswordLink, process.env.JWT_SECRET, function (
      ...
      if (err) {
        return res.status(400).json({
          error: "Link kadaluwarsa. Coba lagi",
        });
      }
      ...
      User.findOne({ resetPasswordLink }, (err, user) => {
        ...
        user.save((err, result) => {
          if (err) {
            return res.status(400).json({
              error: "Error saat mereset password",
            });
          }
          res.json({
